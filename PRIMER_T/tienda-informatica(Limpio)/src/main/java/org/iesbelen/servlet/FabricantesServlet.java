...
public class FabricantesServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		List<FabricantesDTO> fabricanteDTOS = fab.stream()
		...
		if (pathInfo == null || "/".equals(pathInfo)) {
			...
			List<FabricantesDTO> listaOrdenada = fabricanteDTOS;

			String ordenar_por = request.getParameter("ordenar-por");
			String modo_ordenar = request.getParameter("modo-ordenar");

			if(ordenar_por != null && modo_ordenar != null){
				if ("nombre".equals(ordenar_por)) {
					if ("asc".equals(modo_ordenar)) {
						listaOrdenada = fabricanteDTOS.stream().sorted(Comparator.comparing(Fabricante::getNombre)).toList();
					}else if ("desc".equals(modo_ordenar)) {
						listaOrdenada = fabricanteDTOS.stream().sorted(Comparator.comparing(Fabricante::getNombre).reversed()).toList();
					}
				} else if ("codigo".equals(ordenar_por)) {
					if ("asc".equals(modo_ordenar)) {
						listaOrdenada = fabricanteDTOS.stream().sorted(Comparator.comparing(Fabricante::getIdFabricante)).toList();
					} else if ("desc".equals(modo_ordenar)) {
						listaOrdenada = fabricanteDTOS.stream().sorted(Comparator.comparing(Fabricante::getNombre).reversed()).toList();
			...
			if (pathParts.length == 2 && "crear".equals(pathParts[1])) {
				...
				try {
					...
					Optional<FabricantesDTO> fabricantesDTO = fabricante.map(fabricante1 -> new FabricantesDTO(fabricante1, fabDAO.getCountProductos(idFabricante).orElse(0)));

					request.setAttribute("fabricante",fabricantesDTO);
